...
for p in /home/m.caros/work/objectDetection/datasets/CAT3/w_towers_40x40_20p/tower20p_moved_3_*.las; do
  pdal translate $p $p hag_nn --writers.las.extra_dims="HeightAboveGround=float32"
  echo "$p"
done
for p in /home/m.caros/work/objectDetection/datasets/RIBERA/w_towers_40x40_20p/tower20p_moved_3_*.las; do
  pdal translate $p $p hag_nn --writers.las.extra_dims="HeightAboveGround=float32"
  echo "$p"
done
for p in /home/m.caros/work/objectDetection/datasets/BDN/w_towers_40x40_20p/tower20p_moved_3_*.las; do
  pdal translate $p $p hag_nn --writers.las.extra_dims="HeightAboveGround=float32"
  echo "$p"
done
