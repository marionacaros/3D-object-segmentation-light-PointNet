    ...
    def forward(self, input, slice_idx_mat, pool_type, slice_counts):
        ...
        if self.pool_type == "Max_Pool":
            if not input.is_cuda:
                slice_pool_layer.slice_pool_max_forward(input, slice_idx_mat, num_slice, num_batch, channels,
                                                        num_points, out, pool_mask)
                self.pool_mask = pool_mask
            else:
                out = out.cuda()
                pool_mask = pool_mask.cuda()
                slice_pool_layer.slice_pool_max_forward_cuda(input, slice_idx_mat, num_slice, num_batch, channels,
                                                             num_points, out, pool_mask)
        ...
        # Avg_Pool
            ...
            if not input.is_cuda:
                slice_pool_layer.slice_pool_avg_forward(input, slice_idx_mat, slice_counts, num_slice, num_batch,
                                                        channels, num_points, out)
            else:
                out = out.cuda()
                slice_pool_layer.slice_pool_avg_forward_cuda(input, slice_idx_mat, slice_counts, num_slice, num_batch,
                                                             channels, num_points, out)
    ...
    def backward(self, grad_top):
        ...
        if self.pool_type == "Max_Pool":
            if not grad_top.is_cuda:
                slice_pool_layer.slice_pool_max_backward(grad_top, self.pool_mask, num_slice, num_batch, channels,
                                                         grad_bottum)
            else:
                grad_bottum = grad_bottum.cuda()
                slice_pool_layer.slice_pool_max_backward_cuda(grad_top, self.pool_mask, num_slice, num_batch, channels,
                                                              grad_bottum)
        ...
        # Avg_Pool
            ...
            if not grad_top.is_cuda:
                slice_pool_layer.slice_pool_avg_backward(grad_top, self.slice_idx_mat, self.slice_counts, num_slice,
                                                         num_batch, channels, self.num_points, grad_bottum)
            else:
                grad_bottum = grad_bottum.cuda()
                slice_pool_layer.slice_pool_avg_backward_cuda(grad_top, self.slice_idx_mat, self.slice_counts,
                                                              num_slice, num_batch, channels, self.num_points,
                                                              grad_bottum)
